...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      {/* Hero Section */}
      <section className="relative pt-20 pb-32 overflow-hidden bg-gradient-to-br from-white via-primary-50/30 to-accent-50/20">
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-wrap items-center -mx-4">
            <div className={`w-full lg:w-1/2 px-4 mb-12 lg:mb-0 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
              <span className="inline-block py-1.5 px-4 mb-4 text-xs font-semibold text-primary-600 bg-primary-50 rounded-full uppercase tracking-widest animate-fade-in shadow-sm">
                The Future of Fleet Management
              </span>
              <h1 className="text-5xl lg:text-7xl font-bold text-secondary-900 mb-6 leading-tight animate-slide-up">
                Manage your <span className="text-primary-600 relative inline-block">
                  Vehicles
                  <span className="absolute bottom-2 left-0 right-0 h-3 bg-primary-200/40 -z-10 transform -skew-x-12"></span>
                </span> with Precision.
              </h1>
              <p className="text-xl text-secondary-600 mb-10 max-w-lg leading-relaxed animate-slide-up delay-200">
              ...
              <div className="flex flex-wrap gap-4 animate-slide-up delay-300">
                <Link 
                  to="/signup" 
                  className="group px-8 py-4 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-2xl shadow-lg shadow-primary-500/25 transition-all duration-300 active:scale-95 hover:shadow-xl hover:shadow-primary-500/40 flex items-center gap-2 hover:gap-3"
                >
                ...
                <Link 
                  to="/login" 
                  className="px-8 py-4 bg-white border-2 border-secondary-200 text-secondary-900 font-bold rounded-2xl hover:bg-secondary-50 hover:border-primary-300 transition-all duration-300 active:scale-95 shadow-sm hover:shadow-md"
                >
                  Sign In
                </Link>
              </div>
            ...
            <div className={`w-full lg:w-1/2 px-4 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              <div className="relative mx-auto lg:mr-0 max-w-max group">
                <div className="absolute top-0 left-0 -mt-10 -ml-10 w-40 h-40 bg-primary-100 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-pulse group-hover:opacity-90 transition-opacity duration-500"></div>
                <div className="absolute bottom-0 right-0 -mb-10 -mr-10 w-40 h-40 bg-accent-100 rounded-full mix-blend-multiply filter blur-3xl opacity-70 animate-pulse delay-700 group-hover:opacity-90 transition-opacity duration-500"></div>
                <div className="absolute -inset-4 bg-gradient-to-r from-primary-200/20 to-accent-200/20 rounded-[3rem] blur-2xl group-hover:blur-3xl transition-all duration-500"></div>
                <img 
                  className="relative rounded-[3rem] shadow-2xl border border-white/20 transform group-hover:scale-105 transition-transform duration-500" 
                  src={backgroundFleet}
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {[
              ...
              <div 
                key={i} 
                className="group text-center p-6 bg-white rounded-2xl shadow-sm hover:shadow-lg border border-secondary-100 hover:border-primary-200 transition-all duration-300 transform hover:-translate-y-2"
              >
                <div className="w-12 h-12 bg-primary-50 rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:bg-primary-600 group-hover:scale-110 transition-all duration-300">
                  <stat.icon className="text-primary-600 group-hover:text-white text-xl transition-colors" />
                </div>
                <p className="text-4xl font-bold text-secondary-900 mb-2 group-hover:text-primary-600 transition-colors">{stat.value}</p>
      ...
      <section className="py-32 bg-white">
        <div className="container mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-20 animate-fade-in">
            <h2 className="text-4xl lg:text-5xl font-bold text-secondary-900 mb-4">Everything you need to run your fleet</h2>
